...
type EnforcementClient interface {
	...
	Compile(ctx context.Context, namespacedID authn.NamespacedID, action string, kinds ...string) (Checker, error)
	...
	HasAccess(ctx context.Context, namespacedID authn.NamespacedID, action string, resources ...Resource) (bool, error)

	// Experimental: LookupResources returns the resources that the user has access to for the given action.
	LookupResources(ctx context.Context, namespacedID authn.NamespacedID, action string) ([]Resource, error)
}
