		...
		},
		{
			...
			initEnv: func(env *testEnv) {
				env.atVerifier.expectedClaims = &Claims[AccessTokenClaims]{
					Claims: jwt.Claims{Subject: types.NewTypeID(types.TypeAccessPolicy, "3")},
		...
		},
		{
			...
			initEnv: func(env *testEnv) {
				...
				env.idVerifier.expectedClaims = &Claims[IDTokenClaims]{
					Claims: jwt.Claims{Subject: types.NewTypeID(types.TypeUser, "3")},
...
func TestGrpcAuthenticator_authenticateService(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			initEnv: func(env *testEnv) {
				env.atVerifier.expectedClaims = &Claims[AccessTokenClaims]{
					Claims: jwt.Claims{Subject: types.NewTypeID(types.TypeAPIKey, "3")},
					Rest:   AccessTokenClaims{Namespace: "stacks-12"},
				}
			},
